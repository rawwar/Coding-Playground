...
async def get_user(user_id: uuid.UUID):
    with open("test.txt", "a") as f:
        f.write(f"get_user called with {user_id}\n")
    return {
        "id": user_id,
...
app = Cadwyn(api_version_default_value="2002-01-01", versions=version_bundle, title="My amazing API")
...
# APIRoute(path='/changelog', name='generate_changelog', methods=['GET']), 
# Route(path='/openapi.json', name='openapi_jsons', methods=['GET', 'HEAD']), 
# Route(path='/docs', name='swagger_dashboard', methods=['GET', 'HEAD']), 
# Route(path='/redoc', name='redoc_dashboard', methods=['GET', 'HEAD']), 
# Route(path='/openapi.json', name='openapi_jsons', methods=['GET', 'HEAD']), 
# APIRoute(path='/users', name='create_user', methods=['POST']),
# APIRoute(path='/users/{user_id}', name='get_user', methods=['GET']),
# APIRoute(path='/test', name='test', methods=['GET']), 
# APIRoute(path='/users/{user_id}/addresses', name='get_user_addresses', methods=['GET']), 
# Route(path='/openapi.json', name='openapi_jsons', methods=['GET', 'HEAD']), 
# APIRoute(path='/users', name='create_user', methods=['POST']),
# APIRoute(path='/users/{user_id}', name='get_user', methods=['GET']), 
# APIRoute(path='/test', name='test', methods=['GET']), 
